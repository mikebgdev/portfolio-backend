...
async def get_system_metrics(
    hours: int = Query(
        default=1, ge=1, le=24, description="Hours of metrics to retrieve"
    )
):
...
async def get_dashboard_metrics(
    hours: int = Query(
        default=1, ge=1, le=24, description="Hours of metrics for dashboard"
    )
):
        ...
        alerts.append(
            {
                ...
                "severity": (
                    "critical" if request_metrics["error_rate"] > 25 else "warning"
                ),
                "component": "api",
