...
from app.exceptions import DatabaseError, ContentNotFoundError
...
class BaseService(Generic[T], ABC):
    ...
    def get_all_active(self, db: Session) -> List[T]:
        ...
        try:
            ...
            logger.error(f"Database error in {self.model_name} get_all_active: {str(e)}")
    ...
    def add_file_data(self, record: T, file_fields: List[str] = None) -> T:
        ...
        if file_fields is None:
            file_fields = [attr for attr in dir(record) 
