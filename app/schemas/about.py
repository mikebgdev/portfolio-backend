...
class AboutBase(BaseModel):
    ...
    # Multilingual fields
    bio_en: str = Field(
        ..., min_length=10, max_length=5000, description="Biography in English"
    )
    bio_es: Optional[str] = Field(
        None, max_length=5000, description="Biography in Spanish"
    )
    hero_description_en: Optional[str] = Field(
        None, max_length=500, description="Hero description in English"
    )
    hero_description_es: Optional[str] = Field(
        None, max_length=500, description="Hero description in Spanish"
    )
    job_title_en: Optional[str] = Field(
        None, max_length=200, description="Job title in English"
    )
    job_title_es: Optional[str] = Field(
        None, max_length=200, description="Job title in Spanish"
    )
    nationality_en: str = Field(
        ..., min_length=1, max_length=100, description="Nationality in English"
    )
    nationality_es: Optional[str] = Field(
        None, max_length=100, description="Nationality in Spanish"
    )
        ...
        if v:
            ...
            for pattern in suspicious_patterns:
                if pattern.lower() in v.lower():
                    raise ValueError(
                        f"Content contains potentially unsafe elements: {pattern}"
                    )
        return v
...
class AboutResponse(AboutBase):
    ...
    photo_data: Optional[Dict[str, Any]] = Field(
        None, description="Photo file as Base64 data URL"
    )
    ...
    def nationality(self) -> str:
        ...
        return (
            self.nationality_es
            if self.language == "es" and self.nationality_es
            else self.nationality_en
        )
