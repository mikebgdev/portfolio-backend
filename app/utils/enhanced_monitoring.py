...
class MetricsCollector:
    ...
    def record_request(self, method: str, path: str, status_code: int, response_time: float, user_id: Optional[int] = None):
        ...
        with self.lock:
            ...
            if endpoint_metrics['recent_times']:
                endpoint_metrics['avg_response_time'] = sum(endpoint_metrics['recent_times']) / len(endpoint_metrics['recent_times'])
    ...
    def get_database_metrics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            avg_duration = sum(op['duration_ms'] for op in self.database_metrics) / total_ops
            
            return {
                'operations_last_hour': total_ops,
                'avg_duration_ms': round(avg_duration, 2),
                'success_rate': round(successful_ops / total_ops * 100, 2) if total_ops > 0 else 100,
    ...
    def get_system_metrics(self, hours: int = 1) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            avg_memory = sum(m['memory']['percent_used'] for m in recent_metrics) / len(recent_metrics)
            avg_disk = sum(m['disk']['percent_used'] for m in recent_metrics) / len(recent_metrics)
...
class EnhancedHealthChecker:
    ...
    def get_comprehensive_health(self) -> Dict[str, Any]:
        ...
        if (self.last_health_check and self.health_cache and 
            (now - self.last_health_check).total_seconds() < self.cache_duration):
    ...
    def _check_system_resources(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "warnings": warnings,
                "message": "System resources within normal range" if not warnings else "; ".join(warnings)
            }
    ...
    def _check_application_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "warnings": warnings,
                "message": "Application functioning normally" if not warnings else "; ".join(warnings)
            }
    ...
    def _check_security_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if severity_breakdown.get('critical', 0) > 0:
                status = "unhealthy"
                warnings.append(f"Critical security events: {severity_breakdown['critical']}")
            elif severity_breakdown.get('high', 0) > 10:
                status = "warning"
                warnings.append(f"High-severity security events: {severity_breakdown['high']}")
            ...
            return {
                ...
                "warnings": warnings,
                "message": "No security concerns" if not warnings else "; ".join(warnings)
            }
