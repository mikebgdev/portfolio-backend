...
class CacheMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp, cacheable_paths: List[str] = None, default_ttl: int = 300):
        super().__init__(app)
                ...
                return JSONResponse(
                    ...
                    headers={
                        **cached_response['headers'],
                        "X-Cache": "HIT",
                        "X-Cache-Key": cache_key[:16] + "..."  # Truncated key for debugging
                    }
                )
...
class CompressionMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            logger.debug(f"Compressed response: {len(response_body)} -> {len(compressed_body)} bytes (ratio: {compression_ratio:.2f})")
...
class RateLimitEnhancedMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp, requests_per_minute: int = 60, burst_requests: int = 10):
        super().__init__(app)
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            response.headers["X-RateLimit-Remaining"] = str(self.requests_per_minute - len(timestamps))
