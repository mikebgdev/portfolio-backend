...
class RateLimitingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        path = request.url.path
        is_lenient_path = any(path.startswith(lenient_path) for lenient_path in self.lenient_paths)
        current_limit = self.lenient_limit if is_lenient_path else self.requests_per_minute
        
        # Check rate limit
        if self._is_rate_limited(client_ip, current_time, current_limit):
            security_logger.logger.warning("Rate limit exceeded", extra={
    ...
    def _clean_old_entries(self, current_time: float):
        ...
        for ip in list(self.requests_history.keys()):
            self.requests_history[ip] = [
                timestamp for timestamp in self.requests_history[ip]
                if timestamp > cutoff_time
            ]
            if not self.requests_history[ip]:
                del self.requests_history[ip]
    
    def _is_rate_limited(self, client_ip: str, current_time: float, limit: int = None) -> bool:
...
class InputSanitizationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if await self._check_query_params(request):
            return self._create_security_error_response(request, "Suspicious query parameters detected")
    ...
    async def _check_query_params(self, request: Request) -> bool:
        ...
        for key, value in request.query_params.items():
            if self._is_suspicious_input(value):
                security_logger.logger.warning("Suspicious query parameter detected", extra={
    ...
    def _check_headers(self, request: Request) -> bool:
        ...
        if self._is_suspicious_input(user_agent):
            security_logger.logger.warning("Suspicious User-Agent detected", extra={
        ...
        if content_length:
            try:
                ...
                if size > self.max_size:
                    security_logger.logger.warning("Request size limit exceeded", extra={
